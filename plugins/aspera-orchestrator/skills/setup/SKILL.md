...
That command installs or version-updates the plugin and reconciles project policy, migrates supported
...
Omitted flags preserve the installed policy. Use `--no-policy` to remove
...
or legacy Aspera conflict.

Use the bundled scripts only for explicitly requested support:

- Read-only diagnosis: `bash ./scripts/doctor.sh [--workspace PATH]`
...
Installation state schema 5 is a compact managed-file receipt; the root installer
automatically migrates valid schema-1 through schema-4 receipts and records
only the managed policy.
